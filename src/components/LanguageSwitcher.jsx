...
export function LanguageSwitcher() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className="absolute right-0 top-full mt-1.5 w-36 rounded-lg border border-border overflow-hidden z-200"
          style={{
          ...
          {LOCALES.map(({ code, label, short }) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="font-medium text-muted-foreground/50 w-5 shrink-0">{short}</span>
                <span className={cn('font-medium', locale === code ? 'text-foreground' : '')}>{label}</span>
              </div>
              {locale === code && (
